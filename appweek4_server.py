...
engine = create_engine('postgresql://week4_user:[quesito19]@nps-demo-instance.cbiq5o3u59ss.us-east-2.rds.amazonaws.com/strategy')
...
def filter_df(df, Exchange, Margin, start_date, end_date):
    dff = df[(df['Margin'] == Margin) & (df['Exchange'] == Exchange) & (df['Entry time'] >= start_date) & (df['Entry time'] < end_date)]
    return dff
                            ...
                            html.Div(
                                ...
                                children=[
                                    ...
                                    dcc.RadioItems(
                                        id="leverage-select",
                                        options=[
                                            {'label': str(label), 'value':label} for label in df['Margin'].unique()
                                        ],
                                        value=1,
                                        labelStyle={'display': 'inline-block'}
                                    ),
...
def update_pnl(exchange, leverage, start_date, end_date):
    ...
    return {'data': [
            go.Bar(        
                ...
                y= dff[dff['Trade type'] == 'Long']['Pnl (incl fees)'],
                name = 'Long'
            ),
            go.Bar(        
                x= dff[dff['Trade type'] == 'Short']['Entry time'],
                y= dff[dff['Trade type'] == 'Short']['Pnl (incl fees)'],
                name = 'Short'
            )
        ],
        'layout': {
...
def update_balance(exchange, leverage, start_date, end_date):
    dff = filter_df(df, exchange, leverage, start_date, end_date).sort_values(by ='Entry time')
    dff['Balance'] = dff['Profit'].cumsum()
